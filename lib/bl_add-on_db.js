            ...
            function loop(n) {
                return new Promise( (resolve) => {
                    function onRequest(err, response, body) {
                        console.log(repoInfo[n]["src_raw_url"]);
                        if (err) { 
                            console.log("Failed to fetch data from API (err=" + JSON.stringify(err) + ", url=" + repoInfo[n]["src_raw_url"] + ")" );
                            repoInfo[n]["main_src_body"] = null;
                        }
                        else if (response.statusCode != 200) {
                            console.log("Failed to fetch data from API. (status=" + response.statusCode + ")");
                            repoInfo[n]["main_src_body"] = null;
                        }
                        else {
