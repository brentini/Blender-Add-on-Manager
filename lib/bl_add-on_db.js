...
var blAddonDB = {
    ...
    setPages: function (startPage, endPage) {
        if (startPage) { this.startPage = startPage; }
        if (endPage) { this.endPage = endPage; }
    },

    setFileSizes: function (minFileSize, maxFileSize) {
        if (minFileSize) { this.minFileSize = minFileSize; }
        if (maxFileSize) { this.maxFileSize = maxFileSize; }
    },
            ...
            function loop(n) {
                return new Promise( (resolve) => {
                    function onRequest(err, response, body) {
                        if (err) {
                            logger.category('lib').warn("Failed to fetch data from API (err=" + JSON.stringify(err) + ", url=" + repoInfo[n]["src_raw_url"] + ")" );
                            repoInfo[n]["main_src_body"] = null;
                        }
    ...
    parseMainSrc: function (repoInfoList) {
        ...
        return new Promise( (resolve) => {
            ...
            for (var i = 0; i < repoInfoList.length; ++i) {
                if (!repoInfoList[i]['main_src_body']) { continue; }
    ...
    updateDBFile: function (file) {
        ...
        logger.category('lib').info("Updating DB file ...");
    },
    ...
    fetchFromDBServer: function (file) {
        var self_ = this;
        var apiURL = self_._getAPIURL(self_.config);
        ...
        if (proxyURL) {
            ...
            /*
                ...
                */
console.log("test2");
            fs.appendFileSync(self_.addonDBFile, JSON.stringify(res.body, null, '  '));
        }
        else {
